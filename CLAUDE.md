...
The hero is the one block that offers a choice of delivery. Each theme variant carries a `mediaType` of `mux` or `sanity`: `mux` plays the adaptive stream, `sanity` plays the hand-encoded set it replaced — an AV1 `.webm`, an HEVC `.mp4`, and a smaller `.webm` for phones. `hero.groq.ts` projects both paths regardless of the setting, so flipping the toggle in Presentation does not blank the background while the query re-runs, and the Studio hides whichever set is not selected.

`mediaTypeOf` in `packages/sanity-blocks/src/hero/media-type.ts` is the only way to read that choice. It **infers** rather than defaults when the field is absent, because the field arrived after the documents did — a blank value means "read what the variant actually carries", so heroes authored before the toggle keep rendering. It lives outside `hero-video.tsx` on purpose: that is a client module pulling in the Mux player graph, and the Markdown serializer needs the same answer without paying for hls.js.

Because `next/dynamic` resolves only when the Mux element renders, a hero set to `sanity` never downloads the player. That isolation is what makes `/sanity-video-delivery` and `/mux-video-delivery` — two `page` documents cloning the homepage page builder with only `mediaType` flipped — a fair measurement of one delivery path against the other.
